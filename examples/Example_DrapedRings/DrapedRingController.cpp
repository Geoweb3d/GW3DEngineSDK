...
DrapedRingController::DrapedRingController()
{}
...
void DrapedRingController::SetRepresentation(Geoweb3d::IGW3DVectorRepresentationWPtr rep)
{
	rep_ = rep;
}

void DrapedRingController::SetLayer(Geoweb3d::IGW3DVectorLayerWPtr dynamic_layer)
{
	dynamic_layer_ = dynamic_layer;
}

void DrapedRingController::SetLocation(double lon, double lat)
{
	lon_ = lon;
	lat_ = lat;
}

void DrapedRingController::SetDragMode(bool drag_mode)
{
	drag_mode_ = drag_mode;
}
void DrapedRingController::SetWorkingFID(long working_fid)
{
	working_fid_ = working_fid;
}

void DrapedRingController::CaptureAScribPoint()
{
...
void DrapedRingController::StopScribingLine()
{
	scribing_pts_.clear();
}

void DrapedRingController::DeleteFeature()
{
	if (!dynamic_layer_.expired())
	{
		auto ep_layer_locked = dynamic_layer_.lock();
...
bool DrapedRingController::OnStream(Geoweb3d::IGW3DVectorLayerStreamResult* result)
{
			...
			|| geometry->get_GeometryType() == Geoweb3d::gtLINE_STRING)
		{
			...
			else
			{
				...
				if (representation_properties)
				{
					...
					static int inner_rad_property_index = representation_properties->get_DefinitionCollection()->get_IndexByName("INNER_RAD"); //...or, by string

					//this is the optimized way to set the representation property!
					double inner_rad = representation_properties->get_Property(Geoweb3d::Vector::DrapedRingProperties::IndividualProperties::INNER_RAD);
					result->put_VectorRepresentationProperty(rep_, inner_rad_property_index, inner_rad);
...
bool DrapedRingController::OnError(/*todo*/)
{
