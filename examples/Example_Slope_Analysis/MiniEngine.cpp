...
Geoweb3d::IGW3DVectorRepresentationWPtr RepresentAsExtruded( Geoweb3d::IGW3DGeoweb3dSDKPtr sdk_context, Geoweb3d::IGW3DVectorLayerWPtr layer )
{
	...
	MiniEngine::MiniEngine( Geoweb3d::IGW3DGeoweb3dSDKPtr sdk_context ) 
		...
		, slope_analysis_handler_( new SlopeAnalysisHandler( sdk_context ) )
	...
	//	w_env.merge(t_data_source.lock()->get_VectorLayerCollection()->get_AtIndex(ii).lock()->get_Envelope());			
		...
		slope_analysis_handler_->ToggleSlopeAnalysis(camera_);
        return true;
    }
    ...
    void MiniEngine::ProcessEvent( const Geoweb3d::WindowEvent& win_event )
    {
			...
			case win_event.KeyPressed:
			{
				if( !camera_.expired() )
				{
					...
					switch(	win_event.Key.code )
					{
						...
						{
							double inner_rad = slope_analysis_handler_->Get_InnerRadius();
							inner_rad+=10;
							slope_analysis_handler_->Set_InnerRadius(inner_rad);
						}
						break;
						...
						{
							double outer_rad = slope_analysis_handler_->Get_OuterRadius();
							outer_rad+=10;
							slope_analysis_handler_->Set_OuterRadius(outer_rad);
						}
						break;
						...
						{
							slope_analysis_handler_->Set_IncrementTargetedSlope();
						}
						break;

						case Geoweb3d::Key::Num4:
						{
							slope_analysis_handler_->Set_DecrementTargetedSlope();
						}
						break;

						case Geoweb3d::Key::Num3:
						{
							slope_analysis_handler_->Set_IncreaseSpan();
						}
						break;

						case Geoweb3d::Key::Num2:
						{
							slope_analysis_handler_->Set_DecreaseSpan();
						}
						break;
						...
						case Geoweb3d::Key::Num1:
						{
							...
							pres_policy++ ;
							if( pres_policy > Geoweb3d::IGW3DRegionOfInterest::LODPolicy::FullIn_ViewOnly )
							{
								pres_policy = 0;
							}
							switch(pres_policy)
							...
							slope_analysis_handler_->Set_LODPolicy((Geoweb3d::IGW3DRegionOfInterest::LODPolicy) pres_policy);
						}
						break;
        ...
        if(move_fanned_viewshed_with_eye && slope_analysis_handler_ && !slope_analysis_handler_->slope_analysis_.expired() )
        {
            ...
            slope_analysis_handler_->slope_analysis_.lock()->put_AddCamera( camera_ );
            ...
            slope_analysis_handler_->slope_analysis_.lock()->put_Location( lon,lat );
            slope_analysis_handler_->slope_analysis_.lock()->put_Elevation( cloc->get_Z() - 1.0 );
            slope_analysis_handler_->slope_analysis_.lock()->put_Rotation( yaw, 45.0f ,roll);
