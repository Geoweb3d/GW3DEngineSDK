...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
/// <summary> Helper class for bundling information about a VectorLayer needed by a Streamer. </summary>
//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
struct FeatureInfo
{
	...
	FeatureInfo ( long object_id ) : id( object_id )
	{ }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
/// <summary> Custom streamer for modifying a feature's geometry. </summary>
////////////////////////////////////////////////////////////////////////////////////////////////////

class ProjectedTextureStream : public Geoweb3d::IGW3DVectorLayerStream
{
	public:

        /////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary> Enum controls for the Projected Texture's translation mode during OnStream. </summary>
        /////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary> Value Constructor for the ProjectedTextureStream.                         </summary>
        ///
        /// <param name="camera"> Camera object this streamer will use for positional queries.  </param>
        /// 
        /// <returns> A Value Constructed ProjectedTextureStream.                               </returns>
        ////////////////////////////////////////////////////////////////////////////////////////////////////

        ProjectedTextureStream( Geoweb3d::IGW3DCameraWPtr camera = Geoweb3d::IGW3DCameraWPtr() )
            : dragging_( false )
		...
		virtual ~ProjectedTextureStream( )
		{
        ...
        ///           for the Projected Texture is created by the App instance. The single
                        ...
                        case CAMERA_POS_AND_PITCH:
                        {
                            ...
                            {                                    
                                ...
                                if ( auto locked_pixel_query = pixel_query.lock() )
                                {
                                    ...
                                    locked_pixel_query->put_QueryFilter( Geoweb3d::IGW3DWindowCoordinateToGeodeticQuery::QueryFilterType::TERRAIN_AND_ALL_OTHER_GEOMETRY );
    ...
    public:
        ...
        Geoweb3d::IGW3DVectorLayerWPtr projected_texture_vector_layer_being_dragged_;        
...
///           layer and use information from the loaded layer(s) to load a Projected
///           Texture. The projected texture will have two different movement modes,
...
class App : public Geoweb3d::IGW3DWindowCallback
{
	public:
        ...
        {
            if ( streamer_.count() > 0 && streamer_.dragging_ && !streamer_.projected_texture_vector_layer_being_dragged_.expired() )
            {
                if ( auto locked_projected_texture_vector_layer = streamer_.projected_texture_vector_layer_being_dragged_.lock() )
                {
                    locked_projected_texture_vector_layer->Stream( &streamer_ );
                }
            }
        }
                ...
                case win_event.KeyPressed:
                {
                    switch ( win_event.Key.code )
                    {
                        case Geoweb3d::Key::Code::Num1:
                            ...
                            streamer_.mode_ = ProjectedTextureStream::ObjectMovementMode::CAMERA_POS;
                            break;
                        case Geoweb3d::Key::Code::Num2:
                            std::cout << "Changing translation mode to CAMERA_POS_AND_PITCH." << std::endl;
                            streamer_.mode_ = ProjectedTextureStream::ObjectMovementMode::CAMERA_POS_AND_PITCH;
                            break;
				        default:
						    break;
                    }
        ...
        void Run()
        {
            ...
            while ( valid )
            {
                ...
                projected_image_representation_.lock()->get_VectorLayer().lock()->Stream( &streamer_ );
            }
        }
		...
		bool Initialize()
		{
			window_ = sdk_engine_context_->get_WindowCollection()->create_3DWindow( "Moving Projected Texture", GW3D_OVERLAPPED, 10, 10, 1280, 720, 0, Geoweb3d::IGW3DStereoDriverPtr(), this ); 
            ...
            // driver(s) that we can use to gather some latitude and longitude positions from and display the projected texture.
            CreateImageryLayer();
            ...
            // This example app's intended purpose is to show how a single layer can create a projected texture
            // and then select it in the scene to move it around with a couple of different translation modes.
			AddProjectedTexture( "EditableProjectedTexture", longitude, latitude );
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary> Forwards information to CreateProjectedTextureLayer and to 
        ///           CreateProjectedTextureRepresentation to properly create a 
        ///           Projected Texture layer and Representation to visualize it in the map.   </summary>
        /// 
        ...
        bool AddProjectedTexture( const char* layer_name, double longitude, double latitude )
        {
            Geoweb3d::IGW3DVectorLayerWPtr projected_texture_layer = CreateProjectedTextureLayer( layer_name, longitude, latitude );            

            if ( projected_texture_layer.expired() )
            {
                std::cout << "Error creating " << layer_name << std::endl;
                return false;
            }

            if ( !CreateProjectedTextureRepresentation( projected_texture_layer ) )
            {
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary> Creates a Projected Texture Feature held by a singular layer.            </summary>
        /// 
        /// <param name="layer_name"> Custom name of the layer being created.                  </param>
        /// 
		...
		Geoweb3d::IGW3DVectorLayerWPtr CreateProjectedTextureLayer( const char* layer_name, double layer_lon, double layer_lat )
		{
            ...
            // Since we're moving a projected texture via its center point, a gtPOINT geometry type is required.            
            ...
            // This will be where the projected texture's location will be.
            Geoweb3d::GW3DPoint projected_texture_point( layer_lon, layer_lat, 50.0 );

            Geoweb3d::GW3DResult result;
            layer.lock()->create_Feature( default_property_collection, &projected_texture_point, result );
        ...
        ///           This will show how to create a projected texture and assign
		...
		bool CreateProjectedTextureRepresentation( Geoweb3d::IGW3DVectorLayerWPtr layer )
		{
            // For Projected textures there is a "ProjectedImage" driver.
            Geoweb3d::IGW3DVectorRepresentationDriverWPtr projected_texture_rep_driver = sdk_engine_context_->get_VectorRepresentationDriverCollection()->get_Driver( "ProjectedImage" );
            
            // We'll add some default values for the projected image's Representation creation parameters.
            Geoweb3d::IGW3DPropertyCollectionPtr   properties            = projected_texture_rep_driver.lock()->get_PropertyCollection()->create_Clone();
            ...
            properties->put_Property( definition_collection->get_IndexByName( "PITCH" ),                 90.0  );
            properties->put_Property( definition_collection->get_IndexByName( "CAMERA_FIELD_OF_VIEW" ),  vfov  );
            properties->put_Property( definition_collection->get_IndexByName( "CAMERA_NEAR_PLANE" ),     nearp );
            properties->put_Property( definition_collection->get_IndexByName( "CAMERA_FAR_PLANE" ),      farp  );

			Geoweb3d::Vector::RepresentationLayerCreationParameter rep_layer_creation_params;
			rep_layer_creation_params.page_level = 1;
            rep_layer_creation_params.representation_default_parameters = properties;

            projected_image_representation_ = projected_texture_rep_driver.lock()->get_RepresentationLayerCollection()->create( layer, rep_layer_creation_params );
            ...
            // Off of the image collection we can 
            Geoweb3d::IGW3DImageCollectionPtr collection_token = sdk_engine_context_->get_SceneGraphContext()->create_ImageCollection();
            collection_token->create( "..//examples//media//Texture//bullseye_white.png" );
            projected_image_representation_.lock()->put_GW3DFinalizationToken( collection_token->create_FinalizeToken() );
            
            // This example will skip the LineSegmentIntersection and click event handling and just always be assigning the projected texture to the camera.
            streamer_.projected_texture_vector_layer_being_dragged_ = projected_image_representation_.lock()->get_VectorLayer();
            streamer_.dragging_ = true;
            streamer_.SetCamera( camera_ );
            streamer_.insert( 0, projected_image_representation_ );
	...
	private:
        ...
        Geoweb3d::IGW3DVectorRepresentationWPtr projected_image_representation_;
        ProjectedTextureStream  streamer_;
...
///           Runs the Moving Projected Texture example application. </summary>
/// 
