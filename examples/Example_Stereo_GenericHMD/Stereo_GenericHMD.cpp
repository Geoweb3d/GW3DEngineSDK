...
public:
	...
	Geoweb3d::IGW3DStereoDriverPtr FindStereoDriver( Geoweb3d::IGW3DGeoweb3dSDKPtr& sdk_context, const char* driver_name )
	{
		Geoweb3d::IGW3DStereoDriverPtr driver;
		for( unsigned int i = 0; i < sdk_context->get_StereoDriverCollection()->count(); ++i )
		{
			const Geoweb3d::IGW3DStereoDriverWPtr d = sdk_context->get_StereoDriverCollection()->get_AtIndex(i);
			if (!strcmp( driver_name, d.unsafe_get()->get_Name()))
			{
				driver = d.lock();
				break;
			}
    ...
    {
		Geoweb3d::IGW3DStereoDriverPtr driver = FindStereoDriver( sdk_context_, "OpenVR Driver" );
        if (driver)
        {
...
void RunApplication( Geoweb3d::IGW3DGeoweb3dSDKPtr sdk_context )
{
    ...
    if(pengine->LoadConfiguration("Example - OpenVR Stereo"))
    {
