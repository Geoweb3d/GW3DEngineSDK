...
{
    return selection_set_fids_.size();
}
...
bool BuildingClassifier::next( long *ppVal )
{
...
long BuildingClassifier::operator[](unsigned long index)
{
...
long BuildingClassifier::get_AtIndex( unsigned long index )
{
...
bool BuildingClassifier::OnStream( Geoweb3d::IGW3DVectorLayerStreamResult *result )
{
    ...
    Color color_ = Color(1.0, 1.0, 1.0);
    ...
    if(classify_color_)
    {
        ...
        defaultsettings->put_Property(color_red_index,   color_.red  );
        defaultsettings->put_Property(color_green_index, color_.green);
        defaultsettings->put_Property(color_blue_index,  color_.blue );
