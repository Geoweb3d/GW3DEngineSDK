...
#include <iostream>
	...
	bool MiniEngine::LoadData(const char *rootpath)
	{
		...
		if (!ds.expired())
		{
			...
			{
				elev_rep_props_ = elevation_rep_driver.lock()->get_PropertyCollection()->create_Clone();
				elev_rep_props_->put_Property(Geoweb3d::Raster::ElevationParameters::VERTICAL_SCALE, 2.0);
				...
				params.representation_default_parameters = elev_rep_props_;

				auto layer = layer_collection->get_AtIndex(0);

				elev_rep_ = elevation_rep_driver.lock()->get_RepresentationLayerCollection()->create(layer, params);
			...
			case win_event.KeyPressed:
			{
				...
				case Geoweb3d::Key::Q:
				{
					...
					if (rep_l)
					{
						...
						elev_rep_props_->put_Property(Geoweb3d::Raster::ElevationParameters::VERTICAL_SCALE, vertical_exageration_);
						rep_l->put_PropertyCollection(elev_rep_props_);

						printf("VERTICAL_SCALE %4.1f\n", vertical_exageration_);
						filter_out = true;
					}
				}
				break;
