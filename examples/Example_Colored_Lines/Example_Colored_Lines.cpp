...
///           class will exemplify how to properly instantiate an imagery layer and 
///           use information from the loaded layer to draw a Colored Line from the
///           layer's top-left coordinate to its bottom-right coordinate. While the
///           App runs, every 15 seconds the Colored Line's properties will change
///           and print the current property state to std::cout.                     </summary>
////////////////////////////////////////////////////////////////////////////////////////////////////
...
class App
{
	public:
        ...
        ///           Colored lLne layer, feature, and representation.              </summary>
        ///
        ...
        bool Initialize()
        {
            window_ = sdk_engine_context_->get_WindowCollection()->create_3DWindow( "Colored Lines Example", GW3D_OVERLAPPED, 10, 10, 1280, 720, 0, Geoweb3d::IGW3DStereoDriverPtr(), nullptr );
            ...
            // Loads a raster layer via the imagery driver that we can 
            // use to gather some latitude and longitude positions from.
            CreateImageryLayer();
            ...
            // Attempt to draw a horizontal line from the top left of the imagery raster to the bottom right.
            ...
            double line_elev       = raster_envelope.MaxZ + 1.0;
            ...
            // Sets the camera on the window to pitch 90 degrees from the center of the imagery raster source.
            UpdateCamera( center_lon, center_lat, camera_elev );

            // This function will exemplify how to properly add a Colored Line by:
            ...
            AddColoredLine( "ColoredLine", left_most_lon, top_most_lat, line_elev, right_most_lon, bottom_most_lat, line_elev );
            ...
            while ( SUCCEEDED( sdk_engine_context_->draw( window_ ) ) )
            {
                ...
                // Colored Line Representation and print to std::cout its current property states.
                RandomizeColoredLineRepresentationProperties();
            }
        ...
        ///           Colored Line layer will have positional information to draw to and from. </summary>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        void UpdateCamera( double longitude, double latitude, double elevation )
        {
            ...
            // This will face the camera towards the ground.
            float pitch = 90.0f;
            camera_.lock()->get_CameraController()->put_Rotation ( 0.0f, pitch, 0.0f );
        }

        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary> Forwards information to CreateColoredLineLayer and to 
        ///           CreateColoredLineRepresentation to properly create a Colored
		...
		bool AddColoredLine( const char* layer_name, 
                             ...
                             double longitude_end,   double latitude_end,   double elevation_end )
		{
            ...
            // In this example we're going to be using the "ColoredLine" driver explicitly.
            colored_line_rep_driver_ = sdk_engine_context_->get_VectorRepresentationDriverCollection()->get_Driver( "ColoredLine" );
            
            if ( colored_line_rep_driver_.expired() )
            {
                std::cout << "Error getting ColoredLine Vector Representation Driver for layer: " << layer_name << std::endl;
                return false;
            }

			colored_line_layer_ = CreateColoredLineLayer( layer_name, longitude_start, latitude_start, elevation_start, longitude_end, latitude_end, elevation_end );

			if( colored_line_layer_.expired() )
			{
				std::cout << "Error creating Colored Line Layer for: " << layer_name << std::endl;
				return false;
			}			
			
			if( !CreateColoredLineRepresentation( colored_line_layer_ ) )
			{
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary> Takes the information forwarded from AddColoredLine and creates
		...
		Geoweb3d::IGW3DVectorLayerWPtr CreateColoredLineLayer( const char* layer_name, 
                                                               double longitude_start, double latitude_start, double elevation_start,
                                                               double longitude_end,   double latitude_end,   double elevation_end )
		{
            ...
            // Using the Vector Driver we can then create a new VectorDataSource with whatever unique name we want. This name can be used to retrieve it in the future.
            Geoweb3d::IGW3DVectorDataSourceWPtr dataset = vector_driver.lock()->get_VectorDataSourceCollection()->create( "ColoredLinesDataset" );
            ...
            // For Colored Lines, you should use Geoweb3d::gtLINE_STRING_25D. This will create the layer to be used for the feature creation.
			...
			if( !layer.expired() )
			{
                ...
                Geoweb3d::GW3DLineString colored_line_coordinates;
                colored_line_coordinates.put_CoordinateDimension( 3 );
                colored_line_coordinates.put_NumPoints( 2 );
                colored_line_coordinates.put_Point( 0, &start_point );
                colored_line_coordinates.put_Point( 1, &end_point );
                ...
                // After this, however, we will update the default property collection periodically every 15 seconds to show how that's done too in RandomizeColoredLineRepresentationProperties.
                Geoweb3d::IGW3DPropertyCollectionPtr default_property_collection = field_definition->create_PropertyCollection();
                layer.lock()->create_Feature( default_property_collection, &colored_line_coordinates, result );
			}
		...
		bool CreateColoredLineRepresentation( Geoweb3d::IGW3DVectorLayerWPtr layer )
		{
			if( colored_line_rep_driver_.expired() )
			{
				std::cout<< "Error creating ColoredLine vector representation driver." << std::endl;
				return false;
			}
			...
			can_represent = colored_line_rep_driver_.lock()->get_CapabilityToRepresent( layer );
			if( !Geoweb3d::Succeeded( can_represent ) )
			{
				std::cout << "Error: The selected ColoredLine representation driver cannot represent this layer." << std::endl;
				return false;
			}            

			// Method 1: Modifying properties via enums.
			// Geoweb3d::IGW3DPropertyCollectionPtr default_colored_line_properties = colored_line_rep_driver_.lock()->get_PropertyCollection()->create_Clone();
            // default_colored_line_properties->put_Property( Geoweb3d::Vector::ColoredLinesProperties::RED,   0.0 );
            // default_colored_line_properties->put_Property( Geoweb3d::Vector::ColoredLinesProperties::GREEN, 1.0 );
            // default_colored_line_properties->put_Property( Geoweb3d::Vector::ColoredLinesProperties::BLUE,  0.0 );
            // default_colored_line_properties->put_Property( Geoweb3d::Vector::ColoredLinesProperties::ALPHA, 1.0 );
            // default_colored_line_properties->put_Property( Geoweb3d::Vector::ColoredLinesProperties::TRANSLATION_Z_OFFSET_MODE, Geoweb3d::Vector::ABSOLUTE_MODE );
            // default_colored_line_properties->put_Property( Geoweb3d::Vector::ColoredLinesProperties::TRANSLATION_Z_OFFSET, 0.001 );
                        	        
            // Method 2: Modifying properties via get_IndexByName.
            Geoweb3d::IGW3DPropertyCollectionPtr default_colored_line_properties = colored_line_rep_driver_.lock()->get_PropertyCollection()->create_Clone();
            const Geoweb3d::IGW3DDefinitionCollection* definition_collection     = default_colored_line_properties->get_DefinitionCollection();
            ...
            default_colored_line_properties->put_Property( definition_collection->get_IndexByName( "RED"   ), 0.0);
            default_colored_line_properties->put_Property( definition_collection->get_IndexByName( "GREEN" ), 1.0 );
            default_colored_line_properties->put_Property( definition_collection->get_IndexByName( "BLUE"  ), 0.0 );
            default_colored_line_properties->put_Property( definition_collection->get_IndexByName( "ALPHA" ), 1.0 );
            default_colored_line_properties->put_Property( definition_collection->get_IndexByName( "TRANSLATION_Z_OFFSET_MODE" ), Geoweb3d::Vector::ABSOLUTE_MODE );
            default_colored_line_properties->put_Property( definition_collection->get_IndexByName( "TRANSLATION_Z_OFFSET" ),      0.001 );
			
            Geoweb3d::Vector::RepresentationLayerCreationParameter colored_line_rep_layer_creation_params;
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary> Occasionally modifies the Colored Line Representation's properties that
        ///           was created in CreateColoredLineRepresentation. Every 15 seconds it will 
        ...
        void RandomizeColoredLineRepresentationProperties()
        {
            ...
            {
                // Modify the colored_line_rep's property collection.
                Geoweb3d::IGW3DPropertyCollectionPtr       new_colored_line_property_collection = colored_line_rep_.lock()->get_PropertyCollection()->create_Clone();
                const Geoweb3d::IGW3DDefinitionCollection* definition_collection                = new_colored_line_property_collection->get_DefinitionCollection();
                new_colored_line_property_collection->put_Property( definition_collection->get_IndexByName( "RED" ),   static_cast< float >( rand() ) / static_cast< float >( RAND_MAX ) );
                new_colored_line_property_collection->put_Property( definition_collection->get_IndexByName( "GREEN" ), static_cast< float >( rand() ) / static_cast< float >( RAND_MAX ) );
                new_colored_line_property_collection->put_Property( definition_collection->get_IndexByName( "BLUE" ),  static_cast< float >( rand() ) / static_cast< float >( RAND_MAX ) );
                new_colored_line_property_collection->put_Property( definition_collection->get_IndexByName( "LINE_WIDTH" ), rand() % 10 + 1 );
                colored_line_rep_.lock()->put_PropertyCollection( new_colored_line_property_collection );

                // Print out property collection information about the colored line representation.
                auto locked_colored_line_rep = colored_line_rep_.lock();
                if ( locked_colored_line_rep )
                {
                    const Geoweb3d::IGW3DPropertyCollection*   colored_line_property_collection = locked_colored_line_rep->get_PropertyCollection();
                    const Geoweb3d::IGW3DDefinitionCollection* definition_collection            = colored_line_property_collection->get_DefinitionCollection();
                    for ( unsigned int definition_index = 0; definition_index < definition_collection->count(); ++definition_index )
                    {
                        ...
                        colored_line_property_collection->get_AtIndex( definition_index )->to_string( property_value_str );
	...
	private:
        ...
        Geoweb3d::IGW3DVectorRepresentationDriverWPtr colored_line_rep_driver_;
		Geoweb3d::IGW3DVectorLayerWPtr			      colored_line_layer_;
		Geoweb3d::IGW3DVectorRepresentationWPtr	      colored_line_rep_;
...
///           Runs the Colored Line example application.            </summary>
/// 
