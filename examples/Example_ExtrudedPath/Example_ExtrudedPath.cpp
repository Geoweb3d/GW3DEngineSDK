...
///           layer and use information from the loaded layer(s) to draw a Extruded Path
///           from a layer's top-left coordinate to its bottom-right coordinate.
///           While the App runs, every 15 seconds the Extruded Path's properties will
...
public:
    ...
    ///           Extruded Path layer, feature, and representation.             </summary>
    ///
    ...
    bool Initialize()
    {
        ...
        // Attempt to draw a diagonal extruded path from the top left of the imagery raster to the bottom right.
        double left_most_lon      = raster_envelope.MaxX;
        double right_most_lon     = raster_envelope.MinX;
        double bottom_most_lat    = raster_envelope.MinY;
        double top_most_lat       = raster_envelope.MaxY;
        double extruded_path_elev = 1000.0;
        ...
        UpdateCamera( center_lon, top_most_lat + ( ( top_most_lat - bottom_most_lat ) / 2 ), camera_elev );

        // This function will exemplify how to properly add an Extruded Path by:
        ...
        AddExtrudedPath( "ExtrudedPath", left_most_lon, top_most_lat, 0.0, right_most_lon, bottom_most_lat, extruded_path_elev );
        ...
        while ( SUCCEEDED( sdk_engine_context_->draw( window_ ) ) )
        {
            ...
            // Extruded Path Representation and print to std::cout its current property states.
            RandomizeProperties();
        }
    ...
    ///           Extruded Path layer will have positional information to draw to and from. </summary>
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    ///           Extruded Path layer will have elevation information to exemplify draping. </summary>
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void UpdateCamera( double longitude, double latitude, double elevation )
    {
        ...
        // This will pitch the camera towards the ground at an angle to show draping and the curtain.
        float pitch = 45.0f;
        camera_.lock()->get_CameraController()->put_Rotation ( 0.0f, pitch, 0.0f );
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    /// <summary> Forwards information to CreateExtrudedPathLayer and to 
    ///           CreateExtrudedPathRepresentation to properly create an Extruded
    ///           Path layer and Representation to visualize it in the map.                  </summary>
    /// 
	...
	bool AddExtrudedPath( const char* layer_name,
                          ...
                          double longitude_end,   double latitude_end,   double elevation_end )
    {
        ...
        // In this example we're going to be using the "ExtrudedPath" driver explicitly.
        extruded_path_rep_driver_ = sdk_engine_context_->get_VectorRepresentationDriverCollection()->get_Driver( "ExtrudedPath" );
        
        if ( extruded_path_rep_driver_.expired() )
        {
            std::cout << "Error getting ExtrudedPath Vector Representation Driver for layer: " << layer_name << std::endl;
            return false;
        }

        extruded_path_layer_ = CreateExtrudedPathLayer( layer_name, longitude_start, latitude_start, elevation_start, longitude_end, latitude_end, elevation_end );

        if ( extruded_path_layer_.expired() )
        {
            std::cout << "Error creating Extruded Path Layer for: " << layer_name << std::endl;
            return false;
        }

        if ( !CreateExtrudedPathRepresentation( extruded_path_layer_ ) )
        {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    /// <summary> Takes the information forwarded from AddExtrudedPath and creates
    ...
    Geoweb3d::IGW3DVectorLayerWPtr CreateExtrudedPathLayer(const char* layer_name, 
                                                           double longitude_start, double latitude_start, double elevation_start,
                                                           double longitude_end,   double latitude_end,   double elevation_end )
    {
        ...
        Geoweb3d::IGW3DVectorDataSourceWPtr dataset = vector_driver.lock()->get_VectorDataSourceCollection()->create( "ExtrudedPathDataset" );
        ...
        // For Extruded Paths, you should use Geoweb3d::gtLINE_STRING_25D. 
        ...
        // Add some coordinates to the extruded path to draw to and from
        if ( !layer.expired() )
        {
            Geoweb3d::GW3DResult     result;
            ...
            Geoweb3d::GW3DLineString extruded_path_coordinates;
            extruded_path_coordinates.put_CoordinateDimension( 3 );
            extruded_path_coordinates.put_NumPoints( 2 );
            extruded_path_coordinates.put_Point( 0, &start_point );
            extruded_path_coordinates.put_Point( 1, &end_point );
            ...
            // After this, however, we will update the default property collection periodically every 15 seconds to show how that's done too in RandomizeExtrudedPathRepresentationProperties.
            layer.lock()->create_Feature( extruded_path_rep_driver_.lock()->get_PropertyCollection(), &extruded_path_coordinates, result );
        }
    ...
    bool CreateExtrudedPathRepresentation( Geoweb3d::IGW3DVectorLayerWPtr layer )
    {
        if ( extruded_path_rep_driver_.expired() )
        {
            std::cout << "Error creating ExtrudedPath vector representation driver." << std::endl;
            return false;
        }
        ...
        can_represent = extruded_path_rep_driver_.lock()->get_CapabilityToRepresent( layer );
        if ( !Geoweb3d::Succeeded( can_represent ) )
        {
            std::cout << "Error: The selected ExtrudedPath representation driver cannot represent this layer." << std::endl;
            return false;
        }

        // Method 1: Modifying properties via enums.
        // Geoweb3d::IGW3DPropertyCollectionPtr default_extruded_path_properties = extruded_path_rep_driver_.lock()->get_PropertyCollection()->create_Clone();
        // default_extruded_path_properties->put_Property( Geoweb3d::Vector::ExtrudedPathProperties::RED,   0.0 );
        // default_extruded_path_properties->put_Property( Geoweb3d::Vector::ExtrudedPathProperties::GREEN, 1.0 );
        // default_extruded_path_properties->put_Property( Geoweb3d::Vector::ExtrudedPathProperties::BLUE,  0.0 );
        // default_extruded_path_properties->put_Property( Geoweb3d::Vector::ExtrudedPathProperties::ALPHA, 1.0 );
        // default_extruded_path_properties->put_Property( Geoweb3d::Vector::ExtrudedPathProperties::TRANSLATION_Z_OFFSET_MODE, Geoweb3d::Vector::ABSOLUTE_MODE );
        // default_extruded_path_properties->put_Property( Geoweb3d::Vector::ExtrudedPathProperties::TRANSLATION_Z_OFFSET, 0.001 );

        // Method 2: Modifying properties via get_IndexByName.
        Geoweb3d::IGW3DPropertyCollectionPtr default_extruded_path_properties = extruded_path_rep_driver_.lock()->get_PropertyCollection()->create_Clone();
        const Geoweb3d::IGW3DDefinitionCollection* definition_collection = default_extruded_path_properties->get_DefinitionCollection();
        ...
        bool show_curtain = true;
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "LIGHTING_MODE" ), Geoweb3d::Vector::ExtrudedPathProperties::LightingMode::NONE_SHADED );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "RENDER_MODE" ),   show_curtain ? 15 : 11 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "OIT" ),           Geoweb3d::Vector::OITMode::STIPPLE_MODE);

        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "TRANSLATION_OFFSET_HEADING" ),  0.0 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "TRANSLATION_OFFSET_DISTANCE" ), 0.0 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "RED" ),                         0.0 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "GREEN" ),                       1.0 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "BLUE" ),                        0.0 );

        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "ALPHA" ),             1.00 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "DRAPED_LINE_ALPHA" ), 1.00 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "CURTAIN_ALPHA" ),     0.75 );

        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "DRAPED_LINE_COLOR_BIAS" ), -0.5 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "CURTAIN_COLOR_BIAS" ),     -0.5 );

        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "TEXTURE_TO_COLOR_BLEND" ), 0.0 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "TEXTURE_PALETTE_INDEX" ),  -1 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "TEXTURE_MODE_ENABLE" ),    false );

        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "LEFT_DISTANCE" ),  1 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "RIGHT_DISTANCE" ), 1 );

        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "LINE_WIDTH" ),    5 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "OUTLINE_MODE" ), Geoweb3d::Vector::OutlineMode::OUTLINE_ON);

        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "TRANSLATION_Z_OFFSET" ),      0.0 );
        default_extruded_path_properties->put_Property( definition_collection->get_IndexByName( "TRANSLATION_Z_OFFSET_MODE" ), Geoweb3d::Vector::TRANSLATION_Z_OFFSET_MODE::ABSOLUTE_MODE );

        Geoweb3d::Vector::RepresentationLayerCreationParameter extruded_path_rep_layer_creation_params;
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    /// <summary> Occasionally modifies the Extruded Path Representation's properties that
    ///           was created in CreateExtrudedPathRepresentation. Every 15 seconds it will 
    ...
    void RandomizeProperties()
    {
        ...
        {
            // Modify the extruded_path_rep's property collection.
            Geoweb3d::IGW3DPropertyCollectionPtr new_extruded_path_property_collection = extruded_path_rep_.lock()->get_PropertyCollection()->create_Clone();
            auto definition_collection = new_extruded_path_property_collection->get_DefinitionCollection();
            new_extruded_path_property_collection->put_Property( definition_collection->get_IndexByName( "RED" ),               static_cast< float >( rand() ) / static_cast< float >( RAND_MAX ) );
            new_extruded_path_property_collection->put_Property( definition_collection->get_IndexByName( "GREEN" ),             static_cast< float >( rand() ) / static_cast< float >( RAND_MAX ) );
            new_extruded_path_property_collection->put_Property( definition_collection->get_IndexByName( "BLUE" ),              static_cast< float >( rand() ) / static_cast< float >( RAND_MAX ) );                                                      
            new_extruded_path_property_collection->put_Property( definition_collection->get_IndexByName( "ALPHA" ),             static_cast< float >( rand() ) / static_cast< float >( RAND_MAX ) );
            new_extruded_path_property_collection->put_Property( definition_collection->get_IndexByName( "DRAPED_LINE_ALPHA" ), static_cast< float >( rand() ) / static_cast< float >( RAND_MAX ) );
            new_extruded_path_property_collection->put_Property( definition_collection->get_IndexByName( "CURTAIN_ALPHA" ),     static_cast< float >( rand() ) / static_cast< float >( RAND_MAX ) );            
            new_extruded_path_property_collection->put_Property( definition_collection->get_IndexByName( "LINE_WIDTH" ),        rand() % 10 + 3 );
            new_extruded_path_property_collection->put_Property( definition_collection->get_IndexByName( "OUTLINE_MODE" ),      rand() %  2 == 0 ? Geoweb3d::Vector::OutlineMode::OUTLINE_OFF : Geoweb3d::Vector::OutlineMode::OUTLINE_ON);
            new_extruded_path_property_collection->put_Property( definition_collection->get_IndexByName( "OIT" ),               rand() %  2 == 0 ? Geoweb3d::Vector::OITMode::STIPPLE_MODE : Geoweb3d::Vector::OITMode::ALPHA_MODE );

            extruded_path_rep_.lock()->put_PropertyCollection( new_extruded_path_property_collection );

            // Print out property collection information about the Extruded Path Representation.
            auto locked_extruded_path_rep = extruded_path_rep_.lock();
            if ( locked_extruded_path_rep )
            {
                const Geoweb3d::IGW3DPropertyCollection*   extruded_path_property_collection = locked_extruded_path_rep->get_PropertyCollection();
                const Geoweb3d::IGW3DDefinitionCollection* definition_collection             = extruded_path_property_collection->get_DefinitionCollection();
                for ( unsigned int definition_index = 0; definition_index < definition_collection->count(); ++definition_index )
                {
                    ...
                    extruded_path_property_collection->get_AtIndex( definition_index )->to_string( property_value_str );
...
private:
    ...
    Geoweb3d::IGW3DVectorRepresentationDriverWPtr extruded_path_rep_driver_;
    Geoweb3d::IGW3DVectorLayerWPtr			      extruded_path_layer_;
    Geoweb3d::IGW3DVectorRepresentationWPtr	      extruded_path_rep_;
...
///           Runs the Extruded Path example application.           </summary>
/// 
